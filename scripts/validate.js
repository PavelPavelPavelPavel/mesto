...
const setEventListeners = (form, validationConfig) => {
    ...
    inputList.forEach(inputValue => { // пробегаемся по полям формы
         inputValue.addEventListener('input', () => {//навещиваем слушатель событий на инпуты
...
function toggleButtonState (inputList, submitButton, validationConfig) {
    if (hasInvalidInput(inputList) === true) {// проверяем поле на валидность 
